...
#define TO_32BIT(L, n)                    \
    (eluart_toInteger((L), (n)) & 0x0ffffffff  )

#else
#pragma message "32 Bit Integer size..."
#define TO_32BIT(L, n)  (eluart_toInteger((L), (n)))
#endif 
...
static int cpu_w32( lua_State *L )
{
  ...
  addr = ( rt_uint32_t )TO_32BIT( L, 1 );
  data = ( rt_uint32_t )TO_32BIT( L, 2 );
  *( rt_uint32_t* )addr = data;
  return 0;
}
...
static int cpu_r32( lua_State *L )
{
  ...
  // luaL_checkinteger( L, 1 );
  addr = ( rt_uint32_t )TO_32BIT( L, 1 );
  lua_pushinteger( L, ( lua_Integer )( *( rt_uint32_t* )addr ) );
  return 1;
}
...
{
  rt_uint32_t addr;
  rt_uint16_t data = ( rt_uint16_t )TO_32BIT( L, 2 );
...
static int cpu_r16( lua_State *L )
{
  ...
  // luaL_checkinteger( L, 1 );
  addr = ( rt_uint32_t )TO_32BIT( L, 1 );
  lua_pushinteger( L, ( lua_Integer )( *( rt_uint16_t* )addr ) );
  return 1;
}
...
{
  rt_uint32_t addr;
  rt_uint8_t data = ( rt_uint8_t )TO_32BIT( L, 2 );
...
static int cpu_r8( lua_State *L )
{
  ...
  // luaL_checkinteger( L, 1 );
  addr = ( rt_uint32_t )TO_32BIT( L, 1 );
  lua_pushinteger( L, ( lua_Integer )( *( rt_uint8_t* )addr ) );
  return 1;
}
...
static int  notImpementedError(  lua_State *L )
{
 return luaL_error( L, "function not implemented yet" );
}

// Lua: cpu.cli( id, resnum1, [resnum2], ... [resnumn] )
static int cpu_cli( lua_State *L )
{
  return notImpementedError(L);
}
